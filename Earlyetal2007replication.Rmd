...
# center child outcome measures from Fall (to be entered as control vars), to ease interpretation of coefs
child_data$PPVTEPF <- scale(child_data$PPVTEPF, center=T, scale=F)
child_data$WJ1_SSEPF <- scale(child_data$WJ1_SSEPF, center=T, scale=F)
child_data$WJ10_SSEPF <- scale(child_data$WJ10_SSEPF, center=T, scale=F)
...
df$PPVTEPF <- as.numeric(df$PPVTEPF)
df$WJ1_SSEPF <- as.numeric(df$WJ1_SSEPF)
df$WJ10_SSEPF <- as.numeric(df$WJ10_SSEPF)
df$ECERTOTP <- as.numeric(scale(df$ECERTOTP, center=TRUE, scale=FALSE))
...
for(i in 1:m) {
  # run model
  model <- lm(ECERTOTP ~ 0 + Highest + RATIOPSP + CLS_SIZEPS + HOURSWKP + T_RACEP + WHITEP + PERPOORP + LOCATIONP + FULLDAYP, data = l2.amelia.out$imputations[[i]])
...
q1m2.se.out<-NULL
for(i in 1:m) {
  model <- lm(ECERTOTP ~ 0 + Bachelors + RATIOPSP + CLS_SIZEPS + HOURSWKP + T_RACEP + WHITEP + PERPOORP + LOCATIONP + FULLDAYP, data = l2.amelia.out$imputations[[i]])
...
q1m3.se.out<-NULL
for(i in 1:m) {
  model <- lmer(PPVTEPS ~ 0 + Highest + PPVTEPF + LOCATIONP + FULLDAYP + CHGENP + CHRACEP + CHMATEDNP + POORP + (1 | ICPSR_CLASS_ID), 
...
q1m4.se.out<-NULL
for(i in 1:m) {
  model <- lmer(PPVTEPS ~ 0 + Bachelors + PPVTEPF + LOCATIONP + FULLDAYP + CHGENP + CHRACEP + CHMATEDNP + POORP + (1 | ICPSR_CLASS_ID), 
                   data = l1.amelia.out$imputations[[i]])
...
q1m5.se.out<-NULL
for(i in 1:m) {
  model <- lmer(WJ1_SSEPS ~ 0 + Highest + WJ1_SSEPF + LOCATIONP + FULLDAYP + CHGENP + CHRACEP + CHMATEDNP + POORP + (1 | ICPSR_CLASS_ID), 
                   data = l1.amelia.out$imputations[[i]])
...
q1m6.se.out<-NULL
for(i in 1:m) {
  model <- lmer(WJ1_SSEPS ~ 0 + Bachelors + WJ1_SSEPF + LOCATIONP + FULLDAYP + CHGENP + CHRACEP + CHMATEDNP + POORP + (1 | ICPSR_CLASS_ID), 
                   data = l1.amelia.out$imputations[[i]])
...
q1m7.se.out<-NULL
for(i in 1:m) {
  model <- lmer(WJ10_SSEPS ~ 0 + Highest + WJ10_SSEPF + LOCATIONP + FULLDAYP + CHGENP + CHRACEP + CHMATEDNP + POORP + (1 | ICPSR_CLASS_ID), 
                   data = l1.amelia.out$imputations[[i]])
...
q1m8.se.out<-NULL
for(i in 1:m) {
  model <- lmer(WJ10_SSEPS ~ 0 + Bachelors + WJ10_SSEPF + LOCATIONP + FULLDAYP + CHGENP + CHRACEP + CHMATEDNP + POORP + (1 | ICPSR_CLASS_ID), 
                   data = l1.amelia.out$imputations[[i]])
...
Major.dummies <- dummy.code(df$Major) # No reference group (don't include intercept in model)
Highest.dummies <- dummy.code(df$Highest) # No reference group (don't include intercept in model)
...
df$ECE_CD <- ifelse(is.na(df$Major), NA, df$ECE_CD)
df$HS <- ifelse(is.na(df$Highest), NA, df$HS)
...
jags.dat$ECE_CD <- as.vector(by(as.numeric(df$ECE_CD), df$CLASS, mean))
...
jags.dat$HS <- as.vector(by(as.numeric(df$HS), df$CLASS, mean))
...
# add Bachelors
jags.dat$BachelorsY <- ifelse(jags.dat$HS==1, 0, ifelse(jags.dat$AS==1, 0, ifelse(jags.dat$BA==1,1, ifelse(jags.dat$Grad==1, 1, NA))))
jags.dat$BachelorsN <- ifelse(jags.dat$BachelorsY==0, 1, ifelse(jags.dat$BachelorsY==1,0, NA))
...
jags.dat$param.ECE_CD <- mean(jags.dat$ECE_CD, na.rm=TRUE)
...
jags.dat$param.HS <- mean(jags.dat$HS, na.rm=TRUE)
...
jags.dat$param.BachelorsY <- mean(jags.dat$BachelorsY, na.rm=TRUE)
jags.dat$param.BachelorsN <- mean(jags.dat$BachelorsN, na.rm=TRUE)
...
jags.model <- function(){
  for (j in 1:N.class){
    ...
    mu[j] <-
      ...
      # teacher Highest degree (all four codes because model has no intercept)
      b.HS * HS[j] +
      b.AS * AS[j] +
      b.BA * BA[j] +
      b.Grad * Grad[j]
...
params <- c("b.RATIOPSP", "b.CLS_SIZEPS", "b.HOURSWKP", "b.WHITEP", "b.PERPOORP", "b.LOCATIONP", "b.FULLDAYP", 
            "b.TLatino", "b.TOtherMultiracial", "b.HS", "b.AS", "b.BA", "b.Grad",
            "tau" )
...
inits1 <- list(tau= 1, b.RATIOPSP=0, b.CLS_SIZEPS=0, b.HOURSWKP=0, b.WHITEP=0, b.PERPOORP=0, b.LOCATIONP=0, b.FULLDAYP=0, b.TLatino=0, b.TOtherMultiracial=0, b.HS=0, b.AS=0, b.BA=0, b.Grad=0, 
  RATIOPSP=RATIOPSP.inits, CLS_SIZEPS=CLS_SIZEPS.inits, HOURSWKP=HOURSWKP.inits, WHITEP=WHITEP.inits, PERPOORP=PERPOORP.inits, LOCATIONP=LOCATIONP.inits, FULLDAYP=FULLDAYP.inits, TLatino=TLatino.inits, TOtherMultiracial=TOtherMultiracial.inits, HS=HS.inits, AS=AS.inits, BA=BA.inits, Grad=Grad.inits)
...
q1m1.fit <- jags(data = jags.dat, inits = list(inits1, inits2), parameters.to.save = params, 
            model.file = jags.model, n.chains = 2, n.iter = 10000, n.burnin = 1000)
```
...
jags.model <- function(){
  for (j in 1:N.class){
    ...
    mu[j] <-
      ...
      b.BachelorsY * BachelorsY[j] + 
      b.BachelorsN * BachelorsN[j]
    }
  ...
  for (j in 1:N.class){    
      ...
      TOtherMultiracial[j] ~ dbern(param.TOtherMultiracial) # 0 or 1
      # teacher degree
      BachelorsY[j] ~ dbern(param.BachelorsY) # 0 or 1
      BachelorsN[j] ~ dbern(param.BachelorsN) # 0 or 1
  }
  # priors on coefficients
  b.RATIOPSP ~ dnorm(0,.01)
  b.CLS_SIZEPS ~ dnorm(0,.01)
  ...
  b.TOtherMultiracial ~ dnorm(0,.01)
  # teacher highest degree
  b.BachelorsY ~ dnorm(0,.01)
  b.BachelorsN ~ dnorm(0,.01)
...
# -------------------------------------------------
params <- c("b.RATIOPSP", "b.CLS_SIZEPS", "b.HOURSWKP", "b.WHITEP", "b.PERPOORP", "b.LOCATIONP", "b.FULLDAYP", 
            "b.TLatino", "b.TOtherMultiracial", "b.BachelorsY", "b.BachelorsN",
            "tau" )
...
TLatino.inits <- jags.dat$TLatino
TOtherMultiracial.inits <- jags.dat$TOtherMultiracial
BachelorsY.inits <- jags.dat$BachelorsY
BachelorsN.inits <- jags.dat$BachelorsN
...
TLatino.inits[is.na(jags.dat$TLatino)] <- 0
TOtherMultiracial.inits[is.na(jags.dat$TOtherMultiracial)] <- 0
BachelorsY.inits[is.na(jags.dat$BachelorsY)] <- 0
BachelorsN.inits[is.na(jags.dat$BachelorsN)] <- 0
...
TLatino.inits[!is.na(jags.dat$TLatino)] <- NA
TOtherMultiracial.inits[!is.na(jags.dat$TOtherMultiracial)] <- NA
BachelorsY.inits[!is.na(jags.dat$BachelorsY)] <- NA
BachelorsN.inits[!is.na(jags.dat$BachelorsN)] <- NA

# the inits lists are the same, I just need two of them since there are 2 chains
inits1 <- list(tau= 1, b.RATIOPSP=0, b.CLS_SIZEPS=0, b.HOURSWKP=0, b.WHITEP=0, b.PERPOORP=0, b.LOCATIONP=0, b.FULLDAYP=0, b.TLatino=0, b.TOtherMultiracial=0, b.BachelorsY=0, b.BachelorsN=0,
  RATIOPSP=RATIOPSP.inits, CLS_SIZEPS=CLS_SIZEPS.inits, HOURSWKP=HOURSWKP.inits, WHITEP=WHITEP.inits, PERPOORP=PERPOORP.inits, LOCATIONP=LOCATIONP.inits, FULLDAYP=FULLDAYP.inits, TLatino=TLatino.inits, TOtherMultiracial=TOtherMultiracial.inits, BachelorsY=BachelorsY.inits, BachelorsN=BachelorsN.inits)
...
# the inits lists are the same, I just need two of them since there are 2 chains
inits1 <-list(tau= 1, tau.class=1, b.PPVTEPF= 0, b.CHGENP=0, b.POORP=0, b.ChLatino=0, b.ChAfricanAmerican=0, b.ChOtherMultiracial=0, b.CHMATEDNP=0, 
              b.class0=0, b.LOCATIONP=0, b.FULLDAYP=0, b.HS=0, b.AS=0, b.BA=0, b.Grad=0, 
              ...
              LOCATIONP=LOCATIONP.inits, FULLDAYP=FULLDAYP.inits, HS=HS.inits, AS=AS.inits, BA=BA.inits, Grad=Grad.inits)
inits2 <- inits1
...
library(R2jags) 
q1m3.fit <- jags(data = jags.dat, inits = list(inits1, inits2), parameters.to.save = params, 
            model.file = jags.model, n.chains = 2, n.iter = 10000, n.burnin = 4000)
```
...
# -------------------------------------------------
params <- c("b.PPVTEPF", "b.CHGENP", "b.POORP", "b.ChLatino", "b.ChAfricanAmerican", "b.ChOtherMultiracial", "b.CHMATEDNP",
            "b.LOCATIONP", "b.FULLDAYP", "b.BachelorsY", "b.BachelorsN", 
            "b.class0" )
...
# the inits lists are the same, I just need two of them since there are 2 chains
inits1 <-list(tau= 1, tau.class=1, b.PPVTEPF= 0, b.CHGENP=0, b.POORP=0, b.ChLatino=0, b.ChAfricanAmerican=0, b.ChOtherMultiracial=0, b.CHMATEDNP=0, 
              b.class0=0, b.LOCATIONP=0, b.FULLDAYP=0, b.BachelorsY=0, b.BachelorsN=0, 
              PPVTEPF=PPVTEPF.inits, CHGENP=CHGENP.inits, POORP=POORP.inits, ChLatino=ChLatino.inits, ChAfricanAmerican=ChAfricanAmerican.inits, ChOtherMultiracial=ChOtherMultiracial.inits, CHMATEDNP=CHMATEDNP.inits, 
              LOCATIONP=LOCATIONP.inits, FULLDAYP=FULLDAYP.inits, BachelorsY=BachelorsY.inits, BachelorsN=BachelorsN.inits)
inits2 <- inits1
...
library(R2jags) 
q1m4.fit <- jags(data = jags.dat, inits = list(inits1, inits2), parameters.to.save = params, 
            model.file = jags.model, n.chains = 2, n.iter = 10000, n.burnin = 4000)
```
...
# the inits lists are the same, I just need two of them since there are 2 chains
inits1 <-list(tau= 1, tau.class=1, b.WJ1_SSEPF= 0, b.CHGENP=0, b.POORP=0, b.ChLatino=0, b.ChAfricanAmerican=0, b.ChOtherMultiracial=0, b.CHMATEDNP=0, 
              b.class0=0, b.LOCATIONP=0, b.FULLDAYP=0, b.HS=0, b.AS=0, b.BA=0, b.Grad=0, 
...
library(R2jags) 
q1m5.fit <- jags(data = jags.dat, inits = list(inits1, inits2), parameters.to.save = params, 
            model.file = jags.model, n.chains = 2, n.iter = 10000, n.burnin = 4000)
```
...
library(R2jags) 
q1m6.fit <- jags(data = jags.dat, inits = list(inits1, inits2), parameters.to.save = params, 
            model.file = jags.model, n.chains = 2, n.iter = 10000, n.burnin = 4000)
```
...
# the inits lists are the same, I just need two of them since there are 2 chains
inits1 <-list(tau= 1, tau.class=1, b.WJ10_SSEPF= 0, b.CHGENP=0, b.POORP=0, b.ChLatino=0, b.ChAfricanAmerican=0, b.ChOtherMultiracial=0, b.CHMATEDNP=0, 
              b.class0=0, b.LOCATIONP=0, b.FULLDAYP=0, b.HS=0, b.AS=0, b.BA=0, b.Grad=0, 
...
library(R2jags) 
q1m7.fit <- jags(data = jags.dat, inits = list(inits1, inits2), parameters.to.save = params, 
            model.file = jags.model, n.chains = 2, n.iter = 10000, n.burnin = 4000)
```
...
library(R2jags) 
q1m8.fit <- jags(data = jags.dat, inits = list(inits1, inits2), parameters.to.save = params, 
            model.file = jags.model, n.chains = 2, n.iter = 14000, n.burnin = 4000)
```
...
IV <- list("Highest Degree", "Bachelors (Y/N)")
...
for(i in 1:4){
  ...
  betas <- subset(fit.gg, Parameter=="b.HS" | Parameter=="b.AS" | Parameter=="b.BA" | Parameter=="b.Grad")
  ...
  betas <- subset(fit.gg, Parameter=="b.BachelorsY" | Parameter=="b.BachelorsN" )
